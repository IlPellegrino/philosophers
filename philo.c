...
/*   Updated: 2024/06/18 15:49:31 by nromito          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	check_args(char **argv)
{
	...
	if (ft_atol(argv[2]) <= 0 || check_if_digit(argv[1]) == 1)
		return (write(2, "Wrong time to die\n", 19), 1);
	if (ft_atol(argv[3]) <= 0 || check_if_digit(argv[1]) == 1)
		return (write(2, "Wrong time to eat\n", 19), 1);
	if (ft_atol(argv[4]) <= 0 || check_if_digit(argv[1]) == 1)
		return (write(2, "Wrong time to sleep\n", 21), 1);
	if (argv[5] && (ft_atol(argv[5]) < 0 || check_if_digit(argv[1]) == 1))
...
int	main(int argc, char **argv)
{
	...
	if (argc == 5 || argc == 6)
	{
		if (check_args(argv) == 1)
			return (1);
		init_program(philo, &data, argv);
		init_forks(philo, &data);
